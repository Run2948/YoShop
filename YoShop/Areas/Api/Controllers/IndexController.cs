...
namespace YoShop.Areas.Api.Controllers
{
    ...
    public class IndexController : BaseController
    {
        ...
        {
            // 页面元素
            var page = await _fsql.Select<WxappPage>().Where(l => l.WxappId == wxappId).ToOneAsync();
            // 新品推荐
            var newest = await _fsql.Select<Goods>().Where(l => l.WxappId == wxappId && l.IsDelete == 0 && l.GoodsStatus == 10)
                ...
                .ToListAsync();
            // 猜您喜欢
            var best = await _fsql.Select<Goods>().Where(l => l.WxappId == wxappId && l.IsDelete == 0 && l.GoodsStatus == 10).Limit(10)
